...
source(file.path(here("labs","_common.R")))
...
summary(loans_full_schema$homeownership) #categorical

summary(loans_full_schema$application_type) #categorical

summary(loans_full_schema$interest_rate) #numerical
```
...
p_count <- ggplot(loans, aes(x = homeownership)) +
  geom_bar(fill = IMSCOL["pink", "full"]) + #geom_bar just counts a categorical variable 
  labs(x = "Homeownership", y = "Count")
p_proportion <- loans %>%
  count(homeownership) %>%
  mutate(proportion = n / sum(n)) %>%
  ggplot(aes(x = homeownership, y = proportion)) +
  geom_col(fill = IMSCOL["pink", "full"]) + #geom_col lets you pick a category you want to plot on the y axis
...
p_standardized <- ggplot(loans, aes(x = homeownership, fill = application_type)) +
  geom_bar(position = "fill") + #filling along a second dimension 
  scale_fill_manual(values = c(IMSCOL["blue", "full"], IMSCOL["yellow", "full"])) +
